...
create_test_group() {
    ...
    if [[ -d "$GROUP_PATH" ]]; then
        ...
        # Remove old test data files before importing new ones so stale collection
        # files from a previous import don't cause double-execution.
        # group.json is recreated below; only other *.json files are removed.
        shopt -s nullglob
        for old_file in "$GROUP_PATH"/*.json; do
            [[ "$(basename "$old_file")" == "group.json" ]] && continue
            rm -f "$old_file"
        done
    else
        mkdir -p "$GROUP_PATH"
