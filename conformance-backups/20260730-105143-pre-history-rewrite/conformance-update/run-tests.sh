...
json_file_is_postman_collection() {
    ...
    # Accept any Postman-like collection: must be an object with an item[] array
    # that contains at least one object with a "request" field (possibly nested
    # inside folders).  This handles standard Postman v2.1 exports, portman-
    # generated collections, and vendor collections with minimal metadata.
    jq -e '
        type == "object" and
        (.item? | type == "array") and
        ([ .. | objects | select(has("request")) ] | length > 0)
...
filter_postman_collection_by_group() {
    ...
    jq --slurpfile group "$group_json" '
        ...
        # When testCases is empty, run everything — no filtering needed.
...
execute_wfm_tests_with_group() {
    ...
    # not by filename.
    # ONLY take this path when group.json contains at least one non-UUID test ID
    # (e.g. "scenario-onboarding", "step-1.1").  If every ID in group.json is a
    # UUID it means the group was created from a Postman collection — keep going
    # to the Postman path even if scenario-format files happen to live alongside.
    ...
    if [[ ${#group_scenario_files[@]} -gt 0 ]]; then
        local has_native_ids
        has_native_ids=$(jq 'any(.testCases[]?;
            test("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$") | not
        )' "$group_json" 2>/dev/null || echo "false")
        if [[ "$has_native_ids" == "true" ]]; then
            log "📋 Found ${#group_scenario_files[@]} native scenario file(s)"
            run_wfm_scenario_group "$wfm_url" "$group_path" "$group_name"
            return 0
        fi
        info "Scenario files present but group uses Postman IDs — using Postman collection path"
    fi
    ...
    for group_collection in "${group_collections[@]}"; do
        ...
        if [[ "$matched_items" -eq 0 ]]; then
            # Filter removed everything — fall back to running the full collection so a
            # vendor collection with UUIDs that don't perfectly match group.json still runs.
            warn "No items matched group IDs in $(basename "$group_collection") — running full collection"
            cp "$group_collection" "$filtered_collection"
            matched_items=$(postman_item_count "$filtered_collection")
            if [[ "$matched_items" -eq 0 ]]; then
                warn "Skipping empty collection: $(basename "$group_collection")"
        ...
        if [[ $result -eq 0 ]]; then
            ...
            warn "Some tests failed in $(basename "$group_collection") — see report for details"
            executed_collections=$((executed_collections + 1))
        fi
    done

    if [[ "$executed_collections" -eq 0 ]]; then
        error "No runnable Postman items found in group: $group_name"
    fi
...
################################################################################
# Device Supplier - User-Controlled Step Functions
################################################################################
...
PERSONAS:
  ...
  Device Supplier:
    ...
    • User-controlled steps: generate certs → start server → run tests → stop server
    • Prints mock WFM URL so your real device-agent can connect to it
    • Validates conformance assertions on what the device-agent sent
...
WORKFLOW:
  ...
  2. Run run-tests.sh (CLI #2) to execute tests
     ...
     → Device Supplier: Step-by-step user-controlled flow:
         Step 1 - Generate Certificates (creates certs for mock WFM + device-agent)
         Step 2 - Start Mock WFM Server (prints URL to give your device-agent)
         Step 3 - Run Tests (select group; validates what device-agent sent)
         Step 4 - Stop Mock WFM Server
...
run_device_flow() {
    while true; do
        ...
        case "${device_choice,,}" in
            1) device_generate_certs ;;
            2) device_start_server ;;
            3) device_run_tests ;;
            4) device_stop_server ;;
            b|back)
                return 0
                ;;
            *)
