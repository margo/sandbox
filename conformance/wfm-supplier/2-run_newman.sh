...
patch_collection() {
    ...
    local jq_filter_file="$SCRIPT_DIR/prepare_conformance_collection.jq"
    if [[ ! -f "$jq_filter_file" ]]; then
        error_exit "JQ filter file not found: $jq_filter_file"
    fi

    jq -f "$jq_filter_file" "$COLLECTION_RUNTIME" > "$COLLECTION_RUNTIME.tmp"
