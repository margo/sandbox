...
# --------------------------------------------------
if ! docker buildx inspect public-builder >/dev/null 2>&1; then
 docker buildx create --name public-builder --use
else
 docker buildx use public-builder
fi
# --------------------------------------------------
# Build & Push image (NO PROVENANCE)
# --------------------------------------------------
echo "🏗️ Building & pushing Docker image (no provenance)..."
docker buildx build \
 --provenance=false \
 -f "${DOCKERFILE_PATH}" \
 $(printf -- "--tag %s " "${TAGS[@]}") \
